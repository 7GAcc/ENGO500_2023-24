function [x,t]=gather_data_NNFormat_stats(datasets,trainmode,sensor_toggle)
    x=[];
    t=[];
    if nargin<3
    ...
    for i=1:length(datasets.list)
        ...
        Bari=sensor_toggle(10:11,10:11)*datai.bar_data(:,[3,4])';
        ...
        xi=[Acci;
            ...
            Bari;
            Orienti];
        %Use stats instead of raw data
        for m=1:length(xi(:,1))
            xi_stats(m,:)=[mean(xi(m,:)),std(xi(m,:)),skewness(xi(m,:)),kurtosis(xi(m,:))];
        end
        xi=xi_stats;

        % Compile class values in format for NN for this file
        if trainmode
            ti=zeros(length(datasets.classes),length(xi(1,:)));
